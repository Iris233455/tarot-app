...
class SubscriptionProduct {
  ...
  static const premium3Months = SubscriptionProduct(
    id: 'premium_3months',
    title: 'プレミアム3ヶ月プラン',
    description: '3ヶ月プレミアムサービス（10%割引）',
    price: '¥2,700',
    currency: 'JPY',
    amount: 2700.0,
    durationDays: 90,
...
class SubscriptionService {
  ...
  static Future<void> initialize() async {
    debugPrint('🏪 订阅服务初始化开始...');
    
    if (kDebugMode) {
      debugPrint('🧪 开发模式：使用模拟订阅功能');
    }
  ...
  static Future<SubscriptionStatus> getCurrentStatus() async {
    ...
    if (_useSupabaseAsSource) {
      try {
        ...
        debugPrint('🚨 从Supabase获取订阅状态失败，回退到本地数据: $e');
      }
    }
  ...
  static Future<DateTime?> getExpiryDate() async {
    ...
    if (_useSupabaseAsSource) {
      try {
        ...
        debugPrint('🚨 从Supabase获取订阅到期时间失败，回退到本地数据: $e');
      }
    }
  ...
  static Future<bool> purchaseSubscription([String? productId]) async {
    ...
    if (userId == null) {
      debugPrint('❌ 购买失败：用户未登录');
      return false;
    }
    
    // 检查用户类型：匿名用户不能购买订阅
    if (currentUser?.isAnonymous == true) {
      debugPrint('❌ 购买失败：匿名用户不能购买订阅');
      return false;
    }
    ...
    try {
      if (kDebugMode) {
        ...
        debugPrint('🧪 模拟购买流程开始，产品ID: $productId');
        ...
        if (response != null && response['success'] == true) {
          ...
          debugPrint('❌ 模拟购买失败: $error - $message');
          return false;
        }
      }
  ...
  static Future<void> activateSubscriptionWithCode(DateTime expiryDate) async {
    debugPrint('🎫 通过兑换码激活订阅，到期时间: ${expiryDate.toIso8601String()}');
    ...
    if (SupabaseService.currentUserId != null) {
      try {
        ...
        debugPrint('🚨 同步兑换码激活到Supabase失败: $e');
      }
    }
    
    debugPrint('✅ 兑换码订阅激活成功');
  }
  ...
  static Future<void> extendSubscription(int days) async {
    ...
    debugPrint('🎫 延长订阅 $days 天，新到期时间: ${newExpiry.toIso8601String()}');
