...
class RedemptionService {
  static const String _logTag = '🎫 兑换码服务';
    ...
    try {
      ...
      if (!isValidCodeFormat(formattedCode)) {
        return RedemptionResult.failure(
          'invalid_format',
          '引き換えコードが正しくありません。6-12桁の英数字を入力してください',
        );
      }
      ...
      return RedemptionResult.success(
        redemptionCode.days,
        '有効な引き換えコード：${redemptionCode.days}日間のプレミアムサービス',
      );
      
    } catch (e) {
      debugPrint('$_logTag 检查兑换码失败: $e');
      return RedemptionResult.failure(
        'network_error',
        'ネットワークエラーです。インターネット接続をご確認ください',
      );
    }
  }
  ...
  static Future<RedemptionResult> redeemCode(String code) async {
    try {
      ...
      if (currentUser?.isAnonymous == true) {
        return RedemptionResult.failure(
          'anonymous_user',
          'ゲストユーザーは引き換えコードを使用できません。メール登録が必要です',
        );
      }
      ...
      if (result['success'] == true) {
        ...
        return RedemptionResult.success(
          days,
          result['message'] ?? '引き換え完了！',
        );
      } else {
        final error = result['error'] ?? 'unknown_error';
        final message = result['message'] ?? '兑换失败';
  ...
  static Future<List<String>> generateTestCodes({
    ...
    if (!kDebugMode) {
      throw Exception('仅在开发模式下可生成测试兑换码');
    }
