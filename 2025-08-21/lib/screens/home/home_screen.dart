...
class HomeScreen extends HookConsumerWidget {
                ...
                child: Row(
                  children: [
                    ...
                    TextButton(
                      ...
                      child: Text(
                        strings.buttonReconnect,
                        style: TextStyle(color: dynamicTokens.textPrimary),
                      ),
                    ),
                  ],
                ),
                  ...
                  builder: (context, ref, child) {
                    ...
                    return monthlyCardsAsync.when(
                      data: (cardMap) => MonthCalendar(
                        cardMap: cardMap,
                        onDayTap: (date) => _onCalendarDayTap(context, ref, date, cardMap, dynamicTokens),
                        onMonthChanged: (_) {},
                      ),
                      loading: () => SizedBox(
                        height: 280,
                      ...
                      error: (error, stack) => SizedBox(
                        ...
                        child: Center(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.error, color: DynamicTokens.textError, size: 48),
                              const SizedBox(height: 12),
                              Text(strings.messageDataLoadFailed, style: TextStyle(color: DynamicTokens.textError)),
                            ],
                          ),
                        ),
                      ),
  ...
  void _onCalendarDayTap(BuildContext context, WidgetRef ref, DateTime date,
      Map<DateTime, DailyCardInfo?> cardMap, DynamicTokens dynamicTokens) {
    final now = DateTime.now();
    ...
    if (tapped.isAfter(today)) {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
                          content: Text(strings.messageWaitForDate),
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(context),
                child: Text(strings.buttonUnderstand)),
          ],
        ),
      );
      return;
    ...
    if (cardInfo == null) {
      ...
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          content: Text(strings.messageNoHistory),
          actions: [
            TextButton(
                onPressed: () => Navigator.pop(context),
                child: Text(strings.buttonUnderstand)),
    ...
    // 有抽牌，显示牌意，现在数据是完整的
    _showCardDetail(context, cardInfo.card, cardInfo.isUpright, dynamicTokens);
  }
}

// 新增：大牌区的抽牌逻辑和动画
class _HomeDailyCard extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_HomeDailyCard> createState() => _HomeDailyCardState();
}

class _HomeDailyCardState extends ConsumerState<_HomeDailyCard> with TickerProviderStateMixin {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
                            content: Text(strings.messageDailyCardDrawn),
        ...
        if (currentState.card != null && mounted) {
          final dynamicTokens = widget.ref.read(dynamicTokensProvider);
          _showCardDetail(context, currentState.card!, currentState.isUpright, dynamicTokens);
        }
      }
    } catch (e) {
      setState(() {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
                            content: Text(strings.messageDrawFailed),
                          ...
                          child: Transform(
                            ...
                            child: _flipAnimation.value < 0.5
                                ...
                                : Transform(
                                    ...
                                    alignment: Alignment.center,
                                    child: _buildCardFront(dynamicTokens),
                                  ),
                          ),
                        );
                      },
  ...
  Widget _buildCardFront(DynamicTokens dynamicTokens) {
...
class _CardBackPatternPainter extends CustomPainter {
  ...
  _CardBackPatternPainter({required this.patternColor});
...
// 解析文本的第一行和剩余内容
  ...
  void _showCardDetail(BuildContext context, TarotCard card, bool isUpright, DynamicTokens dynamicTokens) {
  showDialog(
    context: context,
    builder: (_) {
            ...
            children: [
              ...
              Padding(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError).withOpacity(0.08),
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(
                      color: isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError,
                      width: 1,
                    ),
                  ),
                  child: Text(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            fontWeight: DynamicTokens.fontWeightRegular, // 不加粗
                            fontSize: DynamicTokens.fontSizeBodyMedium,
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(
                      color: (isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError).withOpacity(0.22),
                    ),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          fontWeight: DynamicTokens.fontWeightRegular, // 不加粗
                          fontSize: DynamicTokens.fontSizeBodyMedium,
                          color: (isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError).withOpacity(0.7),
                        ),
                      ),
                      const SizedBox(height: 8),
                      Builder(
                      ...
                      Builder(
                        builder: (context) {
                          ...
                          if (keywords.isEmpty) {
                            return Text(
                              strings.messageNoKeywords, style: TextStyle(
                          ...
                          return Wrap(
                            ...
                            children: keywords
                                ...
                                .map((keyword) => Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: (isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError).withOpacity(0.16),
                                        borderRadius:
                                            BorderRadius.circular(12),
                                        border: Border.all(
                                          color: (isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError).withOpacity(0.3),
                                        ),
                                      ),
                                      child: Text(
                                        keyword,
                                        style: TextStyle(
                                          fontSize: DynamicTokens.fontSizeBodySmall,
                                          color: isUpright
                                              ? DynamicTokens.textSuccess
                                              : DynamicTokens.textError,
...
class _TodayAISection extends ConsumerWidget {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(ref.watch(appStringsProvider).homeTodayMessage,
                style: TextStyle(
                  fontSize: DynamicTokens.fontSizeTitleMedium,
                  fontWeight: DynamicTokens.fontWeightBold,
                  color: (dynamicTokens.isDark || dynamicTokens.backgroundImage != null)
                    ? dynamicTokens.textPrimary
                    : dynamicTokens.textPrimary.withOpacity(0.8),
                ),
              ),
              const Spacer(),
              // 右侧显示牌名 + 正逆位，提升语境
                ...
                builder: (context, ref, child) {
                  ...
                  return Row(
                    children: [
                      Text(card.nameJa, style: TextStyle(
                        fontSize: DynamicTokens.fontSizeBodySmall, 
                        color: (dynamicTokens.isDark || dynamicTokens.backgroundImage != null) ? DynamicTokens.textGrey600 : dynamicTokens.textTertiary,
                      )),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: isUpright ? DynamicTokens.textSuccess : DynamicTokens.textError, width: 0.8),
                    ),
                    child: Text(
                      isUpright ? strings.labelUpright : strings.labelReversed,
                      style: TextStyle(
  ...
  Future<void> _requestDailyAIReading(WidgetRef ref) async {
    ...
    String userName = strings.labelYou;
    try {
