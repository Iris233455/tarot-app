...
class _MonthCalendarState extends ConsumerState<MonthCalendar> with SingleTickerProviderStateMixin {
  ...
  Future<void> _openYearPicker(BuildContext context) async {
    ...
    final picked = await showDialog<DateTime>(
      ...
      builder: (ctx) {
        return AlertDialog(
          title: Text(ref.watch(appStringsProvider).labelSelectYear),
          content: SizedBox(
            width: 300,
            height: 300,
            child: YearPicker(
  ...
  Widget _buildWeekHeader(BuildContext context) {
    final weekLabels = [
      ref.watch(appStringsProvider).labelMonday,
      ref.watch(appStringsProvider).labelTuesday,
      ref.watch(appStringsProvider).labelWednesday,
      ref.watch(appStringsProvider).labelThursday,
      ref.watch(appStringsProvider).labelFriday,
      ref.watch(appStringsProvider).labelSaturday,
      ref.watch(appStringsProvider).labelSunday,
    ];
    return Consumer(
      builder: (context, ref, child) {
        final dynamicTokens = ref.watch(dynamicTokensProvider);
            ...
            Consumer(
              builder: (context, ref, child) {
                ...
                return Text(
                  ...
                  style: TextStyle(
                    fontSize: 12,
                    fontWeight: isSelected ? DynamicTokens.fontWeightBold : DynamicTokens.fontWeightSemiBold,
                    color: (dynamicTokens.isDark || dynamicTokens.backgroundImage != null)
