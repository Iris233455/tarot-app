...
class AppThemeData {
  ...
  const AppThemeData({
    ...
    this.isDark = false,
    this.textPrimary = const Color(0xFF1A1A1A),  // 使用新的主文本颜色
    this.textSecondary = const Color(0xFF4A4A4A), // 使用新的次文本颜色
  });
}
...
class AppThemes {
  static const Map<String, AppThemeData> themes = {
    ...
    // --- 新テーマ: 大海（オーシャン） ---
          'オーシャン': AppThemeData(
        name: ref.watch(appStringsProvider).themeOcean,
    ...
    // --- 新テーマ: 森林（フォレスト）- 暗色系 ---
          'フォレスト': AppThemeData(
        name: ref.watch(appStringsProvider).themeForest,
    ...
    // --- 新テーマ: 星空（ナイトスカイ） ---
          'ナイトスカイ': AppThemeData(
        name: ref.watch(appStringsProvider).themeNightSky,
    ...
    // --- 新テーマ: 月光（ムーンライト/アメジスト） ---
          'ムーンライト': AppThemeData(
        name: ref.watch(appStringsProvider).themeMoonlight,
    ...
    // --- 追加: クラウド（雲） ---
          'クラウド': AppThemeData(
        name: ref.watch(appStringsProvider).themeCloud,
    ...
    // --- 追加: クリアスカイ（晴空） ---
          'クリアスカイ': AppThemeData(
        name: ref.watch(appStringsProvider).themeClearSky,
...
class ThemeNotifier extends StateNotifier<AppThemeData> {
      ThemeNotifier() : super(AppThemes.themes[ref.watch(appStringsProvider).themeMoonlight]!);
